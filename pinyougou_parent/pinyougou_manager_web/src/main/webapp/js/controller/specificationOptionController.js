...
app.controller('specificationOptionController' ,function($scope,$controller   ,specificationOptionService){	
	...
	$scope.save=function(){				
		...
		if($scope.entity.id!=null){//如果有ID
			obj=specificationOptionService.update( $scope.entity); //修改
		}else{
			obj=specificationOptionService.add( $scope.entity);//增加
		}				
		obj.success(
			function(response){
				if(response.success){
	...
	$scope.del=function() {
        ...
        if (confirm('确定要删除吗？')) {
            ...
            specificationOptionService.del($scope.selectIds).success(
                function (response) {
                    if (response.success) {
