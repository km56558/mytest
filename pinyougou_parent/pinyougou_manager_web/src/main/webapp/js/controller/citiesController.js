...
app.controller('citiesController' ,function($scope,$controller   ,citiesService){	
	...
	$scope.save=function(){				
		...
		if($scope.entity.id!=null){//如果有ID
			obj=citiesService.update( $scope.entity); //修改
		}else{
			obj=citiesService.add( $scope.entity);//增加
		}				
		obj.success(
			function(response){
				if(response.success){
	...
	$scope.del=function() {
        ...
        if (confirm('确定要删除吗？')) {
            ...
            citiesService.del($scope.selectIds).success(
                function (response) {
                    if (response.success) {
	...
	$scope.search=function(page,size){
		citiesService.search(page,size,$scope.searchEntity).success(
			function(response){
